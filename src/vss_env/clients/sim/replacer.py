...
class ReplacerClient(Client):
    ...
    def random_ball_position(self):
        ...
        x = random.uniform(
            -self.__field.LENGTH / 2 + self.__BALL_MARGIN,
            self.__field.LENGTH / 2 - self.__BALL_MARGIN,
        )
        y = random.uniform(
            -self.__field.WIDTH / 2 + self.__BALL_MARGIN,
            self.__field.WIDTH / 2 - self.__BALL_MARGIN,
        )
        return x, y
    ...
    def random_robot_position(self):
        ...
        x = random.uniform(
            -self.__field.LENGTH / 2 + self.__ROBOT_MARGIN,
            self.__field.LENGTH / 2 - self.__ROBOT_MARGIN,
        )
        y = random.uniform(
            -self.__field.WIDTH / 2 + self.__ROBOT_MARGIN,
            self.__field.WIDTH / 2 - self.__ROBOT_MARGIN,
        )
        return x, y
